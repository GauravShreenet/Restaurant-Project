...
public class Sides extends AppCompatActivity {
    ...
    private void showAddFood() {
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(Sides.this);
        ...
        if (saveUri != null){
            ...
            imageFolder.putFile(saveUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                ...
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                    Toast.makeText(Sides.this, "Uploaded !!!", Toast.LENGTH_SHORT).show();
                    imageFolder.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            ...
                            firebaseFirestore.collection("Sides").add(foodMap).addOnCompleteListener(new OnCompleteListener<DocumentReference>() {
                                @Override
                                public void onComplete(@NonNull Task<DocumentReference> task) {
                                    Toast.makeText(Sides.this, "New Food Item is added", Toast.LENGTH_SHORT).show();
                                    recreate();
                                }
                            });
                        }
    ...
    private void deleteFoodItem(EntreeModel entreeModel) {
        firebaseFirestore.collection("Sides").document(entreeModel.getProductId())
                .delete()
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()){
                            Toast.makeText(Sides.this, "Food Item Deleted", Toast.LENGTH_SHORT).show();
                            recreate();
                        }
                    }
                });
    ...
    private void changeImage(EntreeModel entreeModel) {
        if (saveUri != null){
                ...
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                    ...
                    imageFolder.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            ...
                            firebaseFirestore.collection("Sides").document(entreeModel.getProductId()).set(foodMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    Toast.makeText(Sides.this, "Food Item is Updated", Toast.LENGTH_SHORT).show();
                                    recreate();
                                }
                            });
                        }
            ...
            firebaseFirestore.collection("Sides").document(entreeModel.getProductId()).update(foodMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    Toast.makeText(Sides.this, "Food Item is Updated", Toast.LENGTH_SHORT).show();
                    recreate();
