...
public class Mains extends AppCompatActivity {
    private RecyclerView mFirestoreMains;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mFirestoreMains = findViewById(R.id.fireStore_Mains);
        mFirestoreMains.setLayoutManager(new LinearLayoutManager(this));

        menuModelList = new ArrayList<>();
        menuAdapters = new MenuAdapters(this, menuModelList);

        mFirestoreMains.setAdapter(menuAdapters);
        //
    ...
    private void EventChangeListner() {
        ...
        firebaseFirestore.collection("Mains").orderBy("name", Query.Direction.ASCENDING).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
    ...
    private void showAddFood() {
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(Mains.this);
        ...
        if (saveUri != null){
            ...
            imageFolder.putFile(saveUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                ...
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                    Toast.makeText(Mains.this, "Uploaded !!!", Toast.LENGTH_SHORT).show();
                    imageFolder.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            ...
                            firebaseFirestore.collection("Mains").add(foodMap).addOnCompleteListener(new OnCompleteListener<DocumentReference>() {
                                @Override
                                public void onComplete(@NonNull Task<DocumentReference> task) {
                                    Toast.makeText(Mains.this, "New Food Item is added", Toast.LENGTH_SHORT).show();
                                    recreate();
                                }
                            });
                        }
    ...
    private void deleteFoodItem(EntreeModel entreeModel) {
        firebaseFirestore.collection("Mains").document(entreeModel.getProductId())
                .delete()
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()){
                            Toast.makeText(Mains.this, "Food Item Deleted", Toast.LENGTH_SHORT).show();
                            recreate();
                        }
                    }
                });
    ...
    private void changeImage(EntreeModel entreeModel) {
        if (saveUri != null){
                ...
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                    ...
                    imageFolder.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            ...
                            firebaseFirestore.collection("Mains").document(entreeModel.getProductId()).set(foodMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    Toast.makeText(Mains.this, "Food Item is Updated", Toast.LENGTH_SHORT).show();
                                    recreate();
                                }
                            });
                        }
            ...
            firebaseFirestore.collection("Mains").document(entreeModel.getProductId()).update(foodMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull Task<Void> task) {
                    Toast.makeText(Mains.this, "Food Item is Updated", Toast.LENGTH_SHORT).show();
                    recreate();
